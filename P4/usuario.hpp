...
class Usuario{
	public:
		...
		Cadena id()const{return id_user;}
		Cadena nombre()const{return name;}
		Cadena apellidos()const{return apell;}
		Cadena direccion()const{return user_dir;}
		const Tarjetas& tarjetas()const{return list_tarj;}
		void compra(const Articulo& art, unsigned pedido=1);
		const Articulos& compra()const{return list_art;}
		size_t n_articulos()const{return list_art.size();}
		~Usuario();
